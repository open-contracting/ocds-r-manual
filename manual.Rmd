...
The intention of this section is to introduce the reader to the standard, the use cases it was designed for and the basic concepts needed to apply it. Most of the content was taken from the official documentation of the standard; for a more thorough introductory walktrough, please refer to the [getting started guide](https://standard.open-contracting.org/latest/en/getting_started/).
...
[^1]: The author recommends RStudio as the go-to way to run R code and believes it is one major reason behind R's spike in popularity. Consequently, this guide was originally written using RStudio v1.1.453 in macOS High Sierra.
...
Both R and RStudio are available for most of the popular operating systems, including Windows, MacOS and several Linux distributions. Since it is not possible to show instructions for each possible system in here, in this section we will show instructions for Windows and Ubuntu 18.04 Bionic Beaver. Users of other operating systems can find instructions in the official sites for [R](https://www.r-project.org/) and [RStudio](https://www.rstudio.com/products/rstudio/).

Installing R and RStudio on Windows is pretty straightforward: download the installers from [here](https://cloud.r-project.org/bin/windows/base/) and [here](https://www.rstudio.com/products/rstudio/) and run them. Just remember that you have to install R first.

For Ubuntu 18.04 Bionic Beaver there are more steps involved:
...
```
Profit! You should now be able to access Rstudio by running the *rstudio* command or by clicking its icon in the apps menu.
...
The project this document is generated from uses [Packrat](https://rstudio.github.io/packrat/) to keep track of all the libraries that it depends on. This ensures the portability of the software (on import, the appropriate version for the corresponding OS will be installed) and the reproducibility of the analysis. Therefore, playing around with this notebook should be as easy as:

1. Download the notebook in ZIP format from [here](https://github.com/open-contracting/ocds-r-manual/archive/master.zip) and unzip it in a preferred location, or if you prefer to do so, use Git to clone the project:
```{bash, eval=FALSE}
git clone https://github.com/open-contracting/ocds-r-manual.git
```
If you are working on Ubuntu 18.04, install the following libraries:
```{bash notebooklibs, eval=FALSE}
sudo apt-get install libxml2-dev curl libcurl4-openssl-dev libssl-dev libpq-dev g++
```
2. Open RStudio and look for the project file using the menu bar option **File -> Open Project...**

We will also be using a Postgres database to run our analysis, hence we need to install Postgres in case it is missing in our system. On Ubuntu 18.04, we can do so by running:

```{bash, eval=FALSE}
sudo apt install postgresql postgresql-contrib
```

The previous command install the Postgres database engine and creates a *postgres* user as a default account to access our databases.

For Windows, you can download the installer from the [official website](https://www.postgresql.org/download/windows/). The installer is shipped with pgAdmin, an useful graphic tool to manage databases.

# Data acquisition

This section is focused on data loading from diverse sources, which is the first step for any analysis process. We will read data from the four countries this document is concerned with, using them as examples for different data acquisition methods:
...
For Paraguay, Mexico and Colombia sources we are going to use samples prepared for the purpose of this manual. It is worth noting that these samples may not be updated, therefore if you are interested in conducting an analysis of data from these countries you should go to the official publishers:

* Paraguay: [DNCP](https://www.contrataciones.gov.py/datos/api/v2/#!/ocds) and [Ministry of Finance](https://datos.hacienda.gov.py/odmh-api-v1/api-docs/)
* Mexico: [Public Federal Administration](https://datos.gob.mx/busca/dataset/concentrado-de-contrataciones-abiertas-de-la-apf)
* Colombia: [Colombia Compra Eficiente](https://www.colombiacompra.gov.co/transparencia/gestion-documental/datos-abiertos)

## From a REST API

The open contracting dataset from Uruguay is available via a RSS endpoint, which includes URLs to the releases from December 2017 onwards. It is worth noting that since there is not too much data available by the date of publication of the present manual, a deep analysis of Uruguay's contracting data is not possible yet. Having said that, it is still a perfectly suitable example to showcase R's HTTP fetching capabilities. In the following exercise, we will fetch the URLs for the releases of the three latest months.

```{r}
library(XML)
library(dplyr)

# we use the first day of the month to avoid possible range calculation errors
...
In the snippet above, we start by importing R's *XML* library since Uruguay's API responses are in this data format. Since the API receives year and month as parameters, we calculate a sequence of dates representing the three previous months, and build a function to iterate through the months using the *lapply* function.

Inside the iteration function, the *xmlParse* method fetches the content given by the RSS feed url and parses it, returning a tree-like nested array. We then get the root of the parsed structure and seek every element with the *item* tag by calling the *xmlElementsByTagName* method. We convert the result to a data frame and after that to a tibble using *xmlToDataFrame* and *as_tibble* respectively.

The *lapply* function returns a list of the tibbles. To merge them into a single one, we use the function *bind_rows* from the *dplyr* library.
...
At this point we have a list of tibbles, each tibble representing a release. As our next step, we can merge all those tibbles, filling missing values (for example, according to the stage they correspond to, some releases include tender information while some do not) with NA (R's constant for 'Not Available'). We merge the tibbles by using the *bind_rows* function again.

```{r, cache=TRUE}
releases_tb <- bind_rows(releases)
```

As said before, the data available from Uruguay is not big enough yet to make a meaningful analysis, therefore this is as far as we will go with their open contracting data. Nonetheless, the previous exercise can be easily modified to retrieve all the available data from the RSS endpoint to analyze the data in the future.


## From JSON files

We can also read records or releases from a set of JSON files, published by one or more official publishers associated to a country. That is the case of Paraguay, the next country we will be adding to our analysis. As we said before, Paraguayan open contracting data are published by the National Directorate of Public Tender (*Dirección Nacional de Contrataciones Públicas*, DNCP) and the Ministry of Finance (*Ministerio de Hacienda*); as they share ocids, we should be able to use their published data together.
...
3. Create the databases:
  + On Windows, look for pgAdmin in the Start menu and open it. Go to the left panel, click "Servers", then click on the icon of your local PostgreSQL instance. You should see a "Databases" section, right click it and select the option "Create"-> "Database...". On the dialog, name the database as "ocds_colombia" and then click "Save". Repeat to create another database named "ocds_mexico".
  + On Ubuntu 18.04, run `sudo -u postgres createdb ocds_colombia` and `sudo -u postgres createdb ocds_mexico` from a terminal session.
4. Restore the databases: 
  + On Windows, right click on the ocds_colombia database and select the option "Restore...". Use the "Filename" field to find the backup file for Colombia and click on "Restore". Do the same to restore the database ocds_mexico.
  + On Ubuntu 18.04, within a terminal session, from the directory where you downloaded the files, run `pg_restore -d ocds_colombia ocds_colombia.dump` and `pg_restore -d ocds_mexico ocds_mexico.dump`.
...
password <- "postgres" # you should probably change this 2 lines
port <- 5432 # set the port of your Postgres instance, default is 5432
