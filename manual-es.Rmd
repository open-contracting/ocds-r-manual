...
[^1]: El autor recomienda RStudio como la manera más conveniente de ejecutar código R y cree que este IDE es una de las principales razones tras la popularidad de R. En consecuencia, esta guía fue escrita originalmente usando RStudio v1.1.453 en macOS High Sierra.
...
Instalar R y RStudio en Windows es fácil: descarga y ejecuta los instaladores correspondientes desde [aquí](https://cloud.r-project.org/bin/windows/base/) y [aquí](https://www.rstudio.com/products/rstudio/). Recuerda instalar R primero.
...
1. Descarga el proyecto en formato ZIP [aquí](https://github.com/open-contracting/ocds-r-manual/archive/master.zip) y extráelo en un directorio de tu preferencia o, si así lo prefieres, usa Git para clonar el proyecto:
```{bash, eval=FALSE}
...
```
2. Abre RStudio y busca el proyecto usando la opción del menú **File -> Open Project...**
...
En Windows, puedes descargar el instalador desde el [sitio oficial de descargas](https://www.postgresql.org/download/windows/). El instalador incluye pgAdmin, una herramienta gráfica útil para la administración de bases de datos en PostgreSQL.
...
* Los datos de México y Colombia se leerán de columnas JSONB de una base de datos Postgres. Como en este caso se tratará con volúmenes más grandes de datos, se usará como ejemplo de como procesar big data con R usando JSON streams y procesamiento paralelo.
...
* Paraguay: [DNCP](https://www.contrataciones.gov.py/datos/api/v2/#!/ocds) y [Ministerio de Hacienda](https://datos.hacienda.gov.py/odmh-api-v1/api-docs/)
* México: [Administración Pública Federal](https://datos.gob.mx/busca/dataset/concentrado-de-contrataciones-abiertas-de-la-apf)
...
3. Crea las bases de datos:
    + En Windows, busca pgAdmin en el menú de Inicio y ábrelo. Ve al panel izquierdo, haz clic en "Servidores", y luego en el ícono de tu instancia de PostgreSQL local. Deberías ver una sección "Bases de Datos", haz clic derecho en ella y selecciona la opción "Crear" -> "Base de Datos...". En el cuadro de diálogo, escribe el nombre de la base de datos como "ocds_colombia" y luego haz clic en "Guardar". Repite los pasos para crear otra base de datos llamada "ocds_mexico".
    + En Ubuntu 18.04, ejecuta `sudo -u postgres createdb ocds_colombia` y `sudo -u postgres createdb ocds_mexico` desde una terminal.  
3. Restaura las bases de datos:
    + En Windows, haz clic derecho en la base de datos "ocds_colombia" y selecciona la opción "Restaurar...". Usa el campo "Nombre del archivo" para encontrar el archivo de respaldo para Colombia (ocds_colombia.dump) y haz clic en "Restaurar". Repite los pasos para la base de datos ocds_mexico.
    + En Ubuntu 18.04, abre una sessión de terminal y ve al directorio donde descargaste los backups. Ejecuta `pg_restore -d ocds_colombia ocds_colombia_dump` y `pg_restore -d ocds_mexico ocds_mexico.dump`.
